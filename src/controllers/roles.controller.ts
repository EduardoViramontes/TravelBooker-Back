...
import db from "../db";
const mainModel = db.models.Roles
...
export class Roles {
	...
	static async index(req: any, res: Response): Promise<any> {
		try {
			...
			const filtro = await Roles.getFiltro(req.query,mainModel);
	...
	static async options(req: any, res: Response): Promise<any> {
		try {
			...
			for(const doc of docs){
				...
				data.push({
					'id':element.id,
					'option': `(${element.name}) ${element.descripcion ?? ""}`.trim(),
				})
			}
