...
import db from "../db";
const mainModel = db.models.RolePermissions
...
export class RolePermissions {
	...
	static async index(req: any, res: Response): Promise<any> {
		try {
			...
			const filtro = await RolePermissions.getFiltro(req.query,mainModel);
			...
			if (req.query.busquedaLibre !== undefined && req.query.busquedaLibre !== '' && req.query.busquedaLibre !== null) {
				...
				busquedaLibre.push(where(fn("unaccent", col("permissions.model")),    { [Op.iLike]: `%${busquedaLibreTxt}%` }))
				busquedaLibre.push(where(fn("unaccent", col("permissions.descripcion")),    { [Op.iLike]: `%${busquedaLibreTxt}%` }))
				busquedaLibre.push(where(fn("unaccent", col("role.name")),    { [Op.iLike]: `%${busquedaLibreTxt}%` }))
			...
			const relsValidas = [ 'role', 'permissions' ]
	...
	static async store(req: any, res: Response): Promise<any> {
		try {
			...
			const registrosEncontrados = await mainModel.findOne({
				where: {
					idRole:respValid.idRole,
					idPermission:respValid.idPermission,
				},
				paranoid: false
			});
			if(registrosEncontrados != null){
				if(registrosEncontrados.deletedAt == null) return res.status(400).send({ status: false, msg: "Registro existente"});
				await registrosEncontrados.restore();
				return res.status(200).send({ status: true, msg: "Elemento registrado correctamente", data: {id:registrosEncontrados.id}});
			}
